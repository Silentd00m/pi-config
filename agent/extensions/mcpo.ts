...
async function resolveBase(server: string, remote: string, local: string): Promise<string | null> {
...
export default async function (pi: ExtensionAPI) {
  ...
  try {
    ...
    console.warn(`mcpo: could not load config from ${configPath}, using defaults.`);
  }
  ...
  for (const server of config.servers) {
    ...
    let spec: any;
    try {
      spec = await fetch(`${base}/${server}/openapi.json`).then((r) => r.json());
    } catch {
      unavailable.push(server);
      continue;
    }
