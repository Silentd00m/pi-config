...
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), ".pi", "agent", "file-permissions.yaml");
 ...
 */
function buildBwrapCommand(command: string, cwd: string, rules: PermissionRules | null): string {
...
function validatePermissions(perms: unknown, domainPath: string): Map<GuardedToolName, PermissionAction> {
  if (typeof perms !== "object" || perms === null || Array.isArray(perms)) {
    throw new Error(`permissions for "${domainPath}" must be a map of tool: action pairs`);
  }

  const result = new Map<GuardedToolName, PermissionAction>();
  for (const [tool, action] of Object.entries(perms as Record<string, unknown>)) {
    const lowerTool = tool.toLowerCase();
    if (!(GUARDED_TOOLS as readonly string[]).includes(lowerTool)) {
      throw new Error(`Unknown tool "${tool}" for "${domainPath}". Valid: ${GUARDED_TOOLS.join(", ")}`);
    }
    if (action !== "allow" && action !== "ask" && action !== "deny") {
      throw new Error(`Invalid action "${action}" for "${tool}" in "${domainPath}". Valid: allow, ask, deny`);
    }
...
async function parseConfig(raw: string, configPath: string, cwd: string): Promise<LoadedRules> {
...
function evaluateAccess(
  ...
  if (domain) {
    ...
    if (action === "ask") {
      return { allowed: false, action: "ask", reason: `"${toolName}" on "${domain.raw}" requires approval` };
    }
    return { allowed: false, action: "deny", reason: `"${toolName}" is not permitted on "${domain.raw}"` };
  }

  // Default auto-allow: cwd
  if (isSameOrDescendant(normalizePath(cwd), targetPath)) return { allowed: true };
...
function buildPermissionSummary(rules: PermissionRules): string {
  const lines = [`File permissions active from ${configLabel(rules)} (bash sandboxed via bwrap):`];
  for (const domain of rules.domains) {
    const perms = [...domain.permissions.entries()].map(([t, a]) => `${t}:${a}`).join(", ");
    lines.push(`  ${domainLabel(domain)} → {${perms}}`);
  }
  lines.push("Everything not listed is denied. $HOME is a read-only empty sandbox.");
  return lines.join("\n");
}

function buildSystemPromptNotice(rules: PermissionRules): string {
  const domainLines = rules.domains.map((d) => {
    const perms = [...d.permissions.entries()].map(([t, a]) => `${t}:${a}`).join(", ");
...
function buildToolDescription(baseDesc: string, toolName: GuardedToolName, rules: PermissionRules): string {
  ...
  if (allowed.length === 0) return `${baseDesc} This tool is not permitted on any configured path.`;
...
function createPromptGuidelines(toolName: GuardedToolName, rules: PermissionRules): string[] {
  ...
  if (allowed.length > 0) {
    guidelines.push(`Allowed: ${allowed.map((d) => domainLabel(d)).join(", ")}`);
  }
  return guidelines;
}
...
function registerScopedOverrides(pi: ExtensionAPI, cwd: string, rules: PermissionRules): void {
  ...
  pi.registerTool({
    ...
    promptSnippet: "Run bash commands inside a bubblewrap sandbox with restricted home directory.",
    promptGuidelines: [
...
export default function scopedGuardedTools(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      if (rules) {
        console.log(`${chalk.blue("[file-permissions]")} Loaded ${configLabel(rules)} (bash → bwrap)`);
        for (const domain of rules.domains) {
          const perms = [...domain.permissions.entries()].map(([t, a]) => `${t}:${a}`).join(", ");
          console.log(`  ${domainLabel(domain)} → {${perms}}`);
        }
        console.log("  Everything not listed is denied. $HOME is empty+read-only in sandbox.");
        console.log(" ");
      }
    } catch (error) {
      console.log(`${chalk.red("[file-permissions]")} Failed to load config: ${(error as Error).message}\n`);
      if (ctx.hasUI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (["write", "edit"].includes(event.toolName)) {
      ...
      if (typeof rawPath === "string") {
        ...
        if (protectedPaths.includes(abs)) {
          if (ctx.hasUI) ctx.ui.notify(`Blocked: cannot modify permission config file '${rawPath}'`, "warning");
          return { block: true, reason: "Permission config files are read-only and cannot be modified by the agent." };
        }
      }
    }
    ...
    if (event.toolName === "bash") {
      ...
      if (CONFIG_FILE_NAMES.some((name) => command.includes(name))) {
        if (ctx.hasUI) ctx.ui.notify("Blocked: bash command references a permission config file", "warning");
        return { block: true, reason: "bash commands may not reference permission config files." };
      }

      const bashCheck = validateBashCommand(command);
      if (!bashCheck.allowed) {
        if (ctx.hasUI) ctx.ui.notify(bashCheck.reason ?? "Blocked bash command", "warning");
    ...
    if (!GUARDED_TOOLS.includes(event.toolName as GuardedToolName)) return undefined;

    const toolName = event.toolName as GuardedToolName;
    const targetPath = await getTargetPath(toolName, event.input as Record<string, unknown>, ctx.cwd);
    if (!targetPath) return { block: true, reason: `${toolName} requires a path` };
