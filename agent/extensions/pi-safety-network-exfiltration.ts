...
function isHardcodedSecret(value: string): boolean {
  ...
  if (v.length >= 16 && /^[A-Za-z0-9_\-./+=]+$/.test(v) && shannonEntropy(v) >= 3.5) {
    return true;
  }
 ...
 */
const SECRET_PATTERNS: Array<{ pattern: RegExp; captureGroup: number; label: string }> = [
  {
    pattern: /\b(curl|wget)\b.*(-H|--header)\s+['"]?Authorization:\s*(Bearer|Basic|Token)\s+([A-Za-z0-9_\-./+=]{8,})/i,
    captureGroup: 4,
    label: "hardcoded Authorization header",
  },
  {
    pattern: /\b(curl|wget)\b.*[?&](token|key|secret|apikey|api_key)=([A-Za-z0-9_\-./+=]{8,})/i,
    captureGroup: 3,
    label: "hardcoded token in URL",
  },
  {
    // Sensitive file transfer — always block regardless of entropy
    pattern: /\b(curl|wget|scp|rsync)\b.*\.(env|pem|key|p12|pfx|jks|keystore)\b/i,
    captureGroup: -1,
...
const UPLOAD_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  {
    pattern: /\bcurl\b.*(-X\s*(POST|PUT|PATCH|DELETE)|--request\s*(POST|PUT|PATCH|DELETE))/i,
    label: "curl POST/PUT/PATCH/DELETE",
  },
  {
    pattern: /\bcurl\b.*(-d\s|--data[\s=]|--data-raw[\s=]|--data-binary[\s=]|-F\s|--form\s)/i,
...
async function handleFileWriteEvent(
  ...
  if (hardBlockReason) {
    if (ctx.hasUI) ctx.ui.notify(`🚫 Blocked: dangerous content in ${filePath}`, "error");
    return { block: true, reason: `Writing to ${filePath}: ${hardBlockReason}` };
  }
  ...
  if (uploadMatches.length > 0) {
    ...
    const displayPath = filePath.length > 80 ? `…${filePath.slice(-77)}` : filePath;
    ...
    if (!ok) {
      return { block: true, reason: `Blocked by user: file content contains ${uploadMatches.join(", ")}` };
    }
  }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (hardBlockReason) {
      if (ctx.hasUI) ctx.ui.notify("🚫 Blocked: dangerous bash command", "error");
    ...
    if (uploadMatches.length > 0) {
      ...
      if (!ok) {
        return { block: true, reason: `Blocked by user: ${uploadMatches.join(", ")}` };
      }
    }
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (ctx.hasUI) {
      ctx.ui.setStatus("safety-net", ctx.ui.theme.fg("success", "🌐 net-guard"));
    }
  });
}
